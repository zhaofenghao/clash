...
func ServerHandshake(ctx *params.ValueContext, rw net.Conn, authenticator auth.Authenticator) (addr Addr, command Command, err error) {
	...
	if authenticator != nil {
		if Select(buf, Method_USER_PASS) { //支持账密
			// write VER METHOD
			...
			if userLen <= 0 {
				...
				// Verify
				if ok := authenticator.Verify(ctx, user, pass); !ok {
					rw.Write([]byte{1, 1})
					err = ErrAuth
					return
				}
	...
	case CmdConnect, CmdUDPAssociate:
		...
		if localAddr == nil {
			...
			if command == CmdConnect && ctx.LocalIp != "" { //自定义传出本地ip
				_, err = rw.Write(bytes.Join([][]byte{{5, 0, 0}, ParseAddr(ctx.LocalIp)}, []byte{}))
			} else {
...
// Select 方法选择
func Select(methods []byte, method uint8) bool {
